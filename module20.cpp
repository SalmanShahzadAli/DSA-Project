...
void Library()
{
    ...
    Sleep(1000);
    cout << endl;
    cout << "\t\t\t\tIf you are an admin then please press 1 to access admin portal otherwise press anynumber: ";
    cin >> adminPortal;
    if (adminPortal == 1)
    {
        do
        {
            AdminPanel();
            cout << "\t\t\t\t\tDo you want to remain in the admin terminal (Y/N): ";
            cin >> choice;
        } while (choice != 'N' && choice != 'n'); // Correct condition
    }
    else
    {
        do
        {
            StudentPanel();
            cout << "\t\t\t\tDo you want to remain in the Student terminal (Y/N): ";
            cin >> choice;
            Sleep(100);
        } while (choice != 'N' && choice != 'n'); // Correct condition
    }
